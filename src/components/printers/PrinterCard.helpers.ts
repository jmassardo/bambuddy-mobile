...
export const DOOR_SENSOR_MODELS = new Set([
  'X1C',
  'X1',
  'X1E',
  'X2D',
...
export const CHAMBER_FAN_MODELS = new Set([
  'X1C',
  'X1',
  'X1E',
  'X2D',
...
export const SPEED_LEVELS: Record<number, { label: string; percent: string }> = {
...
export const PRINT_GREEN = '#00AE42';
export const PAUSE_AMBER = '#f59e0b';
export const STOP_RED = '#ef4444';
...
export function getEffectiveTrayFill(
  ...
  const inventoryFill =
    spool && spool.label_weight > 0 && spool.weight_used != null
      ? Math.round(
          (Math.max(0, spool.label_weight - spool.weight_used) / spool.label_weight) * 100,
        )
      : null;
