...
function PrinterStatusRow({ printer }: { printer: ApiRecord }) {
  ...
  return (
    <PrinterCard
      ...
      onPress={() =>
        navigation.navigate('PrinterDetail', { id: String(printerId) })
      }
      onCameraPress={() =>
        navigation.navigate('Camera', { id: String(printerId) })
      }
    />
  );
}
...
export default function PrintersDashboardScreen() {
  ...
  const printers = useMemo(
    () => (printersQuery.data ?? []) as ApiRecord[],
    [printersQuery.data],
  );
  const filteredPrinters = useMemo(() => {
    ...
    return printers.filter(printer => {
      ...
      const state = normalizeStatus(
        pickString(printer, ['status', 'state', 'print_status']),
      ).toLowerCase();
      if (filter === 'printing') return state.includes('print');
      if (filter === 'idle')
        return state.includes('idle') || state.includes('ready');
      return (
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            ...
            <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
              {printers.length} printers connected
            </Text>
            <SearchBar
              value={search}
              ...
              onFilterPress={() =>
                setFilter(
                  current =>
                    FILTERS[(FILTERS.indexOf(current) + 1) % FILTERS.length],
                )
              }
            />
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            message={
              printers.length === 0
                ? 'Connect a printer from your Bambuddy server to see live status here.'
