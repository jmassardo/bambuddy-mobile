...
export default function QueueScreen() {
  ...
  const historyQuery = useQuery({
    ...
    queryFn: () => api.getQueueHistory({ limit: 100 }),
  });
  ...
  const data = useMemo(() => {
    if (activeTab === 'history') {
      ...
      const items = Array.isArray(historyData)
        ? historyData
        : pickArray(historyData, ['items', 'results']);
      return items.filter(
    ...
    const source =
      activeTab === 'queue' ? queueQuery.data : pipelinesQuery.data;
    return ((source ?? []) as ApiRecord[]).filter(
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const status = pickString(
            item,
            ['status', 'state', 'result'],
            activeTab === 'queue' ? 'queued' : 'complete',
          );
          const badgeColor = statusColor(status, colors);
          const title = pickString(
            item,
            ['name', 'archive_name', 'job_name', 'pipeline_name'],
            'Unnamed item',
          );
          const subtitle = pickString(
            item,
            ['printer_name', 'target_printer', 'started_by'],
            'No printer assigned',
          );
          return (
            <View
            ...
            >
              ...
              <Text style={[styles.meta, { color: colors.textTertiary }]}>
                {pickString(
                  item,
                  ['note', 'message', 'result_message'],
                  activeTab === 'pipelines'
                    ? 'Pipeline run metadata will appear here.'
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            message="Pull to refresh or switch tabs to review another queue view."
          />
        }
      />
    </View>
