  ...
  return (
    ...
    >
      <Text style={[styles.title, { color: colors.text }]}>
        {pickString(group, ['name'], 'Unnamed group')}
      </Text>
...
export default function UsersScreen() {
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({ title: 'Users & Groups' });
  }, [navigation]);
  const { colors } = useTheme();
  ...
  if (!isAdmin) {
    return (
      <EmptyState
        ...
        message="Only administrators can view users and groups from the mobile app."
      />
    );
  }
  ...
  if (activeQuery.isError) {
    return (
      <ErrorState
        message="Unable to load this admin view."
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          const groups = pickArray(item, ['groups'])
            .map(group => {
          ...
          return (
            ...
            >
              <Text style={[styles.title, { color: colors.text }]}>
                {pickString(item, ['username'], 'Unknown user')}
              </Text>
              <Text style={[styles.meta, { color: colors.textSecondary }]}>
                {pickString(item, ['email'], 'No email')}
              </Text>
              <Text style={[styles.meta, { color: colors.textSecondary }]}>
                Groups: {groups || 'None'}
              </Text>
              <Text style={[styles.meta, { color: colors.textSecondary }]}>
                Last login: {formatDateTime(pickString(item, ['last_login']))}
              </Text>
            </View>
          );
        }}
        ListEmptyComponent={
          <EmptyState
            icon="👥"
            title={`No ${tab} found`}
            message="Create a user or group from the Bambuddy web app to manage it here."
          />
        }
      />
    </View>
