...
export default function InventoryScreen() {
  ...
  React.useLayoutEffect(() => {
    navigation.setOptions({ title: 'Spool Inventory' });
  }, [navigation]);
  ...
  const materials = useMemo(
    () => [
      ...
      ...new Set(
        spools
          .map(spool => pickString(spool, ['material'], 'Unknown'))
          .filter(Boolean),
      ),
    ],
    [spools],
  ...
  const colorsList = useMemo(
    () => [
      ...
      ...new Set(
        spools
          .map(spool => pickString(spool, ['color_name', 'color'], 'Unknown'))
          .filter(Boolean),
      ),
    ],
    [spools],
  ...
  const statuses = useMemo(
    () => [
      ...
      ...new Set(
        spools
          .map(spool => pickString(spool, ['status'], 'Active'))
          .filter(Boolean),
      ),
    ],
    [spools],
  ...
  const filteredSpools = useMemo(() => {
    ...
    return spools.filter(spool => {
      ...
      const matchesSearch =
        !term ||
        [pickString(spool, ['brand', 'vendor']), spoolMaterial, spoolColor]
          .join(' ')
          .toLowerCase()
          .includes(term);
      const matchesMaterial =
        materialFilter === 'All' || spoolMaterial === materialFilter;
      const matchesColor = colorFilter === 'All' || spoolColor === colorFilter;
      const matchesStatus =
        statusFilter === 'All' || spoolStatus === statusFilter;
  ...
  if (spoolsQuery.isError) {
    return (
      <ErrorState
        message="Unable to load spool inventory."
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <FlatList
        ...
        refreshControl={
          <RefreshControl
            refreshing={spoolsQuery.isRefetching}
            onRefresh={() => void spoolsQuery.refetch()}
            ...
            >
              {materials.map(entry => (
                <Chip
                  key={entry}
                  label={entry}
                  selected={materialFilter === entry}
                  onPress={() => setMaterialFilter(entry)}
                />
              ))}
            </ScrollView>
            <ScrollView
              ...
              contentContainerStyle={styles.chipsRow}
            >
              {colorsList.map(entry => (
                <Chip
                  key={entry}
                  label={entry}
                  selected={colorFilter === entry}
                  onPress={() => setColorFilter(entry)}
                />
              ))}
            </ScrollView>
            <ScrollView
        ...
        renderItem={({ item }) => {
          const colorHex = pickString(
            item,
            ['color_hex', 'hex', 'color_code'],
            colors.surfaceHover,
          );
          return (
            <View
            ...
            >
              <View
                style={[
                  styles.swatch,
                  { backgroundColor: colorHex || colors.surfaceHover },
                ]}
              />
              <View style={styles.cardText}>
                <Text style={[styles.title, { color: colors.text }]}>
                  {pickString(item, ['brand', 'vendor'], 'Unknown brand')}
                </Text>
                <Text style={[styles.meta, { color: colors.textSecondary }]}>
                  {pickString(item, ['material'], 'Unknown material')} •{' '}
                  {pickString(item, ['color_name', 'color'], 'Unknown color')}
                </Text>
                <Text style={[styles.meta, { color: colors.textSecondary }]}>
                  Remaining:{' '}
                  {formatWeight(
                    pickString(item, ['weight_remaining_g', 'remaining_g']),
                  )}
                </Text>
              </View>
            </View>
        ...
        ListEmptyComponent={
          <EmptyState
            icon="🧵"
            title="No matching spools"
            message="Add a spool or clear one of the filters."
          />
        }
      />
