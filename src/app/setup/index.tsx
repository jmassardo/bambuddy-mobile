...
export default function SetupScreen() {
  ...
  const setupMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await api.setupAuth({ username: username.trim(), password });
    ...
    onError: (mutationError: Error) => setError(mutationError.message || 'Setup failed.'),
  });
  ...
  return (
    ...
    >
      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}> 
        <Text style={[styles.title, { color: colors.text }]}>Welcome to Bambuddy</Text>
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>Create the first administrator account to finish setup.</Text>
        <TextField label="Admin username" value={username} onChangeText={setUsername} autoCapitalize="none" />
        ...
        <PrimaryButton
          ...
          disabled={username.trim().length === 0 || password.length === 0 || confirm.length === 0}
        />
      </View>
    </KeyboardAvoidingView>
  );
